...
USE OPENMP
USE MPI
USE SCRC, ONLY: SCARC_SETUP, SCARC_SOLVER, SCARC_DUMP_TIME, SCARC_DUMP_CONV, SCARC_DUMP_CPU, SCARC_DUMP_FLOP, SCARC_DUMP_VARI
...
#ifndef WITH_BPH                 
   IF (ICYC > 1e10) EXIT                 !> current own test version only performs 1 time step
#endif
...
PRESSURE_ITERATION_LOOP: DO
   ...
   IF (PRES_METHOD == 'SCARC' .OR. PRES_METHOD == 'USCARC') THEN
#ifdef WITH_BPH
      CALL SCARC_SOLVER(DT, T, SIZE(REQ7), N_REQ7, REQ7)
#else
...
!WRITE(*,*) 'MAIN: IN: HP: PRESSURE_ITERATION', PRESSURE_ITERATIONS, ITERATE_PRESSURE
!IF (PREDICTOR) THEN
!   DO K = MESHES(1)%KBAR, 1, -1
!      WRITE(*,'(6E16.6)') (MESHES(1)%H(I, 1, K), I=1, MESHES(1)%IBAR)
!   ENDDO
!ELSE
!   DO K = MESHES(1)%KBAR, 1, -1
!      WRITE(*,'(6E16.6)') (MESHES(1)%HS(I, 1, K), I=1, MESHES(1)%IBAR)
!   ENDDO
!ENDIF
...
IF (PRES_METHOD == 'SCARC' .OR. PRES_METHOD == 'USCARC') THEN
   CALL SCARC_DUMP_CPU
   CALL SCARC_DUMP_CONV
   CALL SCARC_DUMP_TIME
   CALL SCARC_DUMP_FLOP
   CALL SCARC_DUMP_VARI
ENDIF
